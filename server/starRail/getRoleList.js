...
const GET_ROLE_LIST_URL = "https://act-api-takumi-static.mihoyo.com/common/blackboard/sr_wiki/v1/home/content/list?app_sn=sr_wiki&channel_id=17"

const GET_ROLE_DETAIL = "https://act-api-takumi-static.mihoyo.com/common/blackboard/sr_wiki/v1/content/info?app_sn=sr_wiki&content_id="
...
export const getRoleList = async () => {
  ...
  const roloInfoList = res?.find(item => item.name === "角色")?.list
  if (!roloInfoList) {
    return [];
  }
  ...
  for await (const roleInfo of roloInfoList) {
    const attrInfo = safeParse(safeParse(roleInfo.ext)?.c_18.filter.text, [])?.map(item => item.split('/')) || []
